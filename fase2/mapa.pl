mapa([nodo(dewford), nodo(petalburg), nodo(oldale), nodo(littleroot), nodo(slateport), nodo(rustboro), nodo(verdanturf), nodo(fallarbor),
	...
	aresta(oldale, slateport, 15), aresta(verdanturf, mauville, 10), aresta(slateport, mauville, 25), aresta(mauville, lavaridge, 10), aresta(lavaridge, fallarbor, 15),        
	aresta(rustboro, fallarbor, 35), aresta(fallarbor, mauville, 40), aresta(mauville, fortree, 40), aresta(fortree, lilycove, 30), aresta(lilycove, mossdeep, 15),        
...
%aresta(nodoA, nodoB, distancia(km))

adjacente(A, B, D):- mapa(_, L), member(aresta(A,B,D), L).
adjacente(A, B, D):- mapa(_, L), member(aresta(B,A,D), L).
