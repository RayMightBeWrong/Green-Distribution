mapa([nodo(dewford), nodo(petalburg), nodo(oldale), nodo(littleroot), nodo(slateport), nodo(rustboro), nodo(verdanturf), nodo(fallarbor),
	...
	aresta(rustboro, fallarbor, 35, 0.8), aresta(fallarbor, mauville, 40, 0.9), aresta(mauville, fortree, 40, 0.9), aresta(fortree, lilycove, 30, 0.9), aresta(lilycove, mossdeep, 15, 0.9),        
